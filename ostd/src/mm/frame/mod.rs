...
// `AnyPage` that is surely untyped page, so it must be a valid page that is
...
impl TryFrom<AnyPage> for AnyFrame {
    type Error = AnyPage;

    /// Try converting a [`AnyPage`] into the statically-typed [`Frame`].
    ///
    /// If the dynamic page is not used as an untyped page frame, it will
    /// return the dynamic page itself as is.
    fn try_from(page: AnyPage) -> core::result::Result<Self, Self::Error> {
